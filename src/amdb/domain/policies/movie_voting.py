...
class MovieVotingPolicy(Policy):
    ...
    def check_can_create_full_movie_vote(
        ...
        cannot_create_full_movie = (
            not is_active and self.must_be_active or
            not is_verified and self.must_be_verified or
            datetime.utcnow() - created_at <
            timedelta(days=self.required_days_since_registration) or
            vote_count < self.required_vote_count
        )
